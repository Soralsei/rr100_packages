...
from rr100_rl.utils.helpers import (
    deep_update,
...
from rr100_rl.exceptions import (
    GetLinkStateError,
    GetModelStateError,
...
class GazeboConnection:
    ...
    def __init__(  # noqa: C901
        self, reset_world_or_sim=ResetType.WORLD, max_retry=20, retry_rate=5, log_reset=True
    ):
    ...
    def reset_sim(self):
        ...
        """
        if self._reset_world_or_sim == ResetType.SIMULATION:
            if self._log_reset:
                rospy.logwarn("SIMULATION RESET")
            self._reset_simulation()
        elif self._reset_world_or_sim == ResetType.WORLD:
            if self._log_reset:
                rospy.logwarn("WORLD RESET")
            self._reset_world()
        elif self._reset_world_or_sim == ResetType.NONE:
